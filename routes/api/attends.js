...
/* GET attend by meal */
router.get('/meal/:meal_id', authenticateJWT, async (req, response) => {
  const client = await pool.connect();
  client.query('SELECT * FROM attends WHERE meal_id = $1', [req.params.meal_id])
    .then(result => {
...
router.post('/:id', authenticateJWT, async (req, response) => {
  ...
  `, [meal_id, user_id, status, status, meal_id, user_id, meal_id])
    .then(ans => {
      ...
      } else { // no notification on unattend
        return response.status(201).json(ans.rows);
      }
    })
    .catch(error => {
