...
/////////////////
const getMealsStats = async (days, response) => {
...
const getUsersStat = async (days, response) => {
...
router.get('/statsUsers/:days?', async (req, response) => {
  ...
  const stats = await getUsersStat(req.params.days || 1, response);
...
router.get('/statsMeals/:days?', async (req, response) => {
  console.log(`get system stats`);
  const stats = await getMealsStats(req.params.days || 1, response);
  console.log(`Stats: ${stats.length}`);
  const resp = stats;
  return response.json(resp);
});
/////////////////
const getMealsToday = async (response) => {
...
const countUsers = async (response) => {
...
router.get('/health', async (req, response) => {
  ...
  const meals = await getMealsToday(response);
  const totalUsersRes = await countUsers(response);
...
router.post('/notification', async (req, response) => {
  ...
  console.log(`notification to ${userId}, text: ${text}`);
...
router.post('/mail', async (req, response) => {
  const text = req.body.text;
  const userId = req.body.userId;
  console.log(`mail to ${userId}, text: ${text}`);
  return response.json('Notification sent.');
});
